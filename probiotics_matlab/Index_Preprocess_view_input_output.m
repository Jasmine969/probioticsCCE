...
Z = zscore([ori_time,T,X]);
req_ind = 1:271;
time = Z(req_ind,1); T = Z(req_ind,2); X = Z(req_ind,3);
tag = tag(req_ind); s = s(req_ind);
ori_time = ori_time(req_ind);
...
hold off;h_leg=legend({'$\tilde{t}$','$\tilde{T}_\mathrm{d}$', ...
    '$\tilde{X}$'},'Position', [0.3352 0.2249 0.2976 0.1105], ...
...
h4 = axes(); hold on;
plot(ori_time(tag),s(tag),'v','color',green,'markersize',7,'MarkerFaceColor',green); % ground truth
plot(ori_time(~tag), s(~tag), '.','Color',green,'MarkerSize',6); % itp label
